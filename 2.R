...
for (i in as.character(date)) {
  result <- rbind(result,read.csv(paste("D:/Rworkplace/hist_data/",format(as.Date(i,format='%Y%m%d'),format='%Y%m%d'),"-",format(as.Date(i,format='%Y%m%d')+1,format='%Y%m%d'),"-1.csv",sep = '')
...
png(filename = paste("D:/Rworkplace/pic/",date[length(date)],"交易量趋势.png",sep=''),width = 600*1.35,
    height = 400*1.35)
...
png(filename = paste("D:/Rworkplace/pic/",date[length(date)],"指标趋势.png",sep=''),width = 600*1.35,
    height = 400*1.35)
